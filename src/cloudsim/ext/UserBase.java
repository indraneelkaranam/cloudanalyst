...
public class UserBase extends CloudSim implements GeoLocatable {
	...
	/** Constructor .*/
	public UserBase(String name) throws Exception {
		super(name);
	...
	public UserBase(String name, 
		...
		this.peakHours[0] = peakHours[0] ;
		this.peakHours[1] = peakHours[1];
	...
	private int getOnlineUsers(double time) {
		int avgUsers;
		use++;
		//System.out.println("Time requested "+time+" "+peakHours[0]+" "+peakHours[1]);
		...
		//System.out.println("time(UserBase.java) "+time+ "returning "+temp+" use "+use);
		return temp;
